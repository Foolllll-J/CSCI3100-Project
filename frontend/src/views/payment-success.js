import React from 'react'
import { Link } from 'react-router-dom'

import { Helmet } from 'react-helmet'

import './payment-success.css'

const PaymentSuccess = (props) => {
  return (
    <div className="payment-success-container">
      <Helmet>
        <title>Payment-success - MARTCUHK</title>
        <meta property="og:title" content="Payment-success - MARTCUHK" />
      </Helmet>
      <div className="payment-success-navbar">
        <header
          data-role="Header"
          className="payment-success-header max-width-container"
        >
          <div className="payment-success-navbar1">
            <div className="payment-success-container1">
              <Link to="/search" className="payment-success-navlink">
                <img
                  alt="search3271286"
                  src="data:image/svg+xml;base64,"
                  className="payment-success-image"
                />
              </Link>
              <input
                type="text"
                placeholder="search"
                className="payment-success-textinput input"
              />
            </div>
            <div className="payment-success-middle">
              <div className="payment-success-left">
                <span className="navbar-link">SHOP</span>
                <span className="navbar-link">LOOKBOOK</span>
                <span className="navbar-link">SPECIAL</span>
              </div>
              <Link
                to="/"
                className="payment-success-logo-center navbar-logo-title"
              >
                MARTCUHK
              </Link>
              <div className="payment-success-right">
                <span className="navbar-link">ABOUT</span>
                <span className="navbar-link">BLOG</span>
                <span className="navbar-link">CONTACT</span>
              </div>
            </div>
            <div className="payment-success-icons">
              <Link to="/shopping-cart" className="payment-success-navlink1">
                <img
                  alt="iconsbxscart3271299"
                  src="data:image/svg+xml;base64,"
                  className="payment-success-image1"
                />
              </Link>
              <img
                alt="iconsbxsheartcircle3271300"
                src="data:image/svg+xml;base64,"
                className="payment-success-image2"
              />
              <img
                alt="AccountCircle3271301"
                src="data:image/svg+xml;base64,"
                className="payment-success-image3"
              />
            </div>
          </div>
          <div data-role="BurgerMenu" className="payment-success-burger-menu">
            <svg viewBox="0 0 1024 1024" className="payment-success-icon">
              <path d="M128 554.667h768c23.552 0 42.667-19.115 42.667-42.667s-19.115-42.667-42.667-42.667h-768c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667zM128 298.667h768c23.552 0 42.667-19.115 42.667-42.667s-19.115-42.667-42.667-42.667h-768c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667zM128 810.667h768c23.552 0 42.667-19.115 42.667-42.667s-19.115-42.667-42.667-42.667h-768c-23.552 0-42.667 19.115-42.667 42.667s19.115 42.667 42.667 42.667z"></path>
            </svg>
          </div>
          <div data-role="MobileMenu" className="payment-success-mobile-menu">
            <div className="payment-success-nav">
              <div className="payment-success-container2">
                <span className="payment-success-logo-center1">MOBILLIO</span>
                <div
                  data-role="CloseMobileMenu"
                  className="payment-success-close-mobile-menu"
                >
                  <svg
                    viewBox="0 0 1024 1024"
                    className="payment-success-icon02"
                  >
                    <path d="M810 274l-238 238 238 238-60 60-238-238-238 238-60-60 238-238-238-238 60-60 238 238 238-238z"></path>
                  </svg>
                </div>
              </div>
              <div className="payment-success-middle1">
                <span className="payment-success-text06">SHOP</span>
                <span className="payment-success-text07">LOOKBOOK</span>
                <span className="payment-success-text08">SPECIAL</span>
                <span className="payment-success-text09">ABOUT</span>
                <span className="payment-success-text10">BLOG</span>
                <span className="payment-success-text11">CONTACT</span>
              </div>
            </div>
            <div>
              <svg
                viewBox="0 0 950.8571428571428 1024"
                className="payment-success-icon04"
              >
                <path d="M925.714 233.143c-25.143 36.571-56.571 69.143-92.571 95.429 0.571 8 0.571 16 0.571 24 0 244-185.714 525.143-525.143 525.143-104.571 0-201.714-30.286-283.429-82.857 14.857 1.714 29.143 2.286 44.571 2.286 86.286 0 165.714-29.143 229.143-78.857-81.143-1.714-149.143-54.857-172.571-128 11.429 1.714 22.857 2.857 34.857 2.857 16.571 0 33.143-2.286 48.571-6.286-84.571-17.143-148-91.429-148-181.143v-2.286c24.571 13.714 53.143 22.286 83.429 23.429-49.714-33.143-82.286-89.714-82.286-153.714 0-34.286 9.143-65.714 25.143-93.143 90.857 112 227.429 185.143 380.571 193.143-2.857-13.714-4.571-28-4.571-42.286 0-101.714 82.286-184.571 184.571-184.571 53.143 0 101.143 22.286 134.857 58.286 41.714-8 81.714-23.429 117.143-44.571-13.714 42.857-42.857 78.857-81.143 101.714 37.143-4 73.143-14.286 106.286-28.571z"></path>
              </svg>
              <svg
                viewBox="0 0 877.7142857142857 1024"
                className="payment-success-icon06"
              >
                <path d="M585.143 512c0-80.571-65.714-146.286-146.286-146.286s-146.286 65.714-146.286 146.286 65.714 146.286 146.286 146.286 146.286-65.714 146.286-146.286zM664 512c0 124.571-100.571 225.143-225.143 225.143s-225.143-100.571-225.143-225.143 100.571-225.143 225.143-225.143 225.143 100.571 225.143 225.143zM725.714 277.714c0 29.143-23.429 52.571-52.571 52.571s-52.571-23.429-52.571-52.571 23.429-52.571 52.571-52.571 52.571 23.429 52.571 52.571zM438.857 152c-64 0-201.143-5.143-258.857 17.714-20 8-34.857 17.714-50.286 33.143s-25.143 30.286-33.143 50.286c-22.857 57.714-17.714 194.857-17.714 258.857s-5.143 201.143 17.714 258.857c8 20 17.714 34.857 33.143 50.286s30.286 25.143 50.286 33.143c57.714 22.857 194.857 17.714 258.857 17.714s201.143 5.143 258.857-17.714c20-8 34.857-17.714 50.286-33.143s25.143-30.286 33.143-50.286c22.857-57.714 17.714-194.857 17.714-258.857s5.143-201.143-17.714-258.857c-8-20-17.714-34.857-33.143-50.286s-30.286-25.143-50.286-33.143c-57.714-22.857-194.857-17.714-258.857-17.714zM877.714 512c0 60.571 0.571 120.571-2.857 181.143-3.429 70.286-19.429 132.571-70.857 184s-113.714 67.429-184 70.857c-60.571 3.429-120.571 2.857-181.143 2.857s-120.571 0.571-181.143-2.857c-70.286-3.429-132.571-19.429-184-70.857s-67.429-113.714-70.857-184c-3.429-60.571-2.857-120.571-2.857-181.143s-0.571-120.571 2.857-181.143c3.429-70.286 19.429-132.571 70.857-184s113.714-67.429 184-70.857c60.571-3.429 120.571-2.857 181.143-2.857s120.571-0.571 181.143 2.857c70.286 3.429 132.571 19.429 184 70.857s67.429 113.714 70.857 184c3.429 60.571 2.857 120.571 2.857 181.143z"></path>
              </svg>
              <svg
                viewBox="0 0 602.2582857142856 1024"
                className="payment-success-icon08"
              >
                <path d="M548 6.857v150.857h-89.714c-70.286 0-83.429 33.714-83.429 82.286v108h167.429l-22.286 169.143h-145.143v433.714h-174.857v-433.714h-145.714v-169.143h145.714v-124.571c0-144.571 88.571-223.429 217.714-223.429 61.714 0 114.857 4.571 130.286 6.857z"></path>
              </svg>
            </div>
          </div>
        </header>
      </div>
      <div className="payment-success-main">
        <div className="payment-success-speakers section-container1">
          <div className="payment-success-max-width max-content-container">
            <div className="payment-success-heading-container">
              <h1 className="payment-success-text12 Heading-1">
                Payment Successful
              </h1>
            </div>
            <div className="payment-success-speakers-container"></div>
            <span className="payment-success-text13">
              Thank you for your payment! Your transaction has been successfully
              processed. A confirmation receipt has been sent to your email. You
              can now continue to browse or return to the homepage. If you have
              any questions, please contact customer service.
            </span>
          </div>
        </div>
      </div>
      <div className="payment-success-footer">
        <div className="max-width-container">
          <footer className="payment-success-footer1">
            <div className="payment-success-container3">
              <h3 className="payment-success-text14 Heading-3">MARTCUHK</h3>
              <span className="payment-success-text15">
                The Chinese University of Hong Kong, Ma Liu Shui, Shatin, Hong
                Kong
              </span>
              <span className="payment-success-text16">(852) 1234-5678</span>
              <span className="payment-success-text17">
                contact@cse.cuhk.edu.com
              </span>
            </div>
            <div className="payment-success-links-container">
              <div className="payment-success-container4">
                <span className="payment-success-text18">Categories</span>
                <span className="payment-success-text19">Collections</span>
                <span className="payment-success-text20">Desks</span>
                <span className="payment-success-text21">Furniture</span>
                <span className="payment-success-text22">Lamps</span>
                <span className="payment-success-text23">Plants</span>
                <span className="payment-success-text24">Decoration</span>
              </div>
              <div className="payment-success-container5">
                <span className="payment-success-text25">Company</span>
                <span className="payment-success-text26">Shop</span>
                <span className="payment-success-text27">Lookbook</span>
                <span className="payment-success-text28">Specials</span>
                <span className="payment-success-text29">About</span>
                <span className="payment-success-text30">Blog</span>
              </div>
              <div className="payment-success-container6">
                <span className="payment-success-text31">Resources</span>
                <span className="payment-success-text32">Contact us</span>
                <Link to="/past-orders" className="payment-success-navlink2">
                  Order
                </Link>
                <Link to="/track-order" className="payment-success-navlink3">
                  Track your order
                </Link>
                <span className="payment-success-text33">
                  Shipping &amp; Delivery
                </span>
              </div>
            </div>
          </footer>
        </div>
      </div>
    </div>
  )
}

export default PaymentSuccess
